...
ように見える」状態を弾く。
  ...
  verify_web.py <port> <screenshot_out.png> [width] [height]

ビューポート寸法は省略可。既定は 1280x720 (横画面)。スマホ縦画面の見た目を
確認したいときは 390 844 のように渡す。判定基準はビューポートに依らず同じ。
...
def main() -> int:
    if len(sys.argv) not in (3, 5):
        sys.exit("usage: verify_web.py <port> <screenshot_out.png> [width] [height]")
    port, out_png = sys.argv[1], sys.argv[2]
    width = int(sys.argv[3]) if len(sys.argv) == 5 else 1280
    ...
    with sync_playwright() as p:
        ...
        # wasmの取得とGodotのブートに時間がかかる。CIのようにGPUが無く
        # ソフトウェアレンダリング(SwiftShader)になる環境では40MB近い
        # wasmのコンパイルがさらに遅いため、WEB_BOOT_MSで伸ばせるようにする。
        page.wait_for_timeout(int(os.environ.get("WEB_BOOT_MS", "15000")))
