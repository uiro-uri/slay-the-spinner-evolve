...
## 決戦(段9のゴール)のノードに打つ星の印。マップで既に使っている色
## (進める先の緑・現在地の青・敵数と脅威の赤・報酬ピップの金・柱の紫)の
## どれとも隣り合わない暖色にする。金(ffcc00)と紛れないよう橙側へ寄せてあり、
## そもそも決戦は報酬ピップが出ない唯一のノードなので同じ画面に金は並ばない。
## 明るい地(進める先の緑)の上では色だけで 3:1 を取れないので、読めるかは
## 柱の印と同じく縁取りが担う(tests/test_contrast.gd)。
const MAP_BOSS := Color("ff8420")
