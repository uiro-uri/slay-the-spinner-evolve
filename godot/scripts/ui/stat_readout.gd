...
##
## 先頭4行は勝敗をほぼ決める複合量の**硬さ・寿命**(守りと持久)と
## **攻め力・弾き**(削って倒す/壁へ押し込む)。いずれもPartPreviewと同じ定義を
## 共有する——**報酬カードが約束する行を、取った後もそのまま出す**のがこの並びの
## 目的だから。生の4本(重さ/大きさ/反発/初期回転)はその材料でしかなく、
...
##
## 攻め力・弾きの2行は 2026-08-08 夜のサイクルで同じ理由から足した。硬さ・寿命を
## 塞いだ後も**攻めの2軸だけが取った瞬間に消えていた**(詳細は attack_index の doc)。
## この2行はCLIには無かった側なので、今回はCLIを実UIへ合わせている。
...
## 攻めの2行だけは規則が違う。倍率(素のコマ=1.00)なので**出発点は必ず下端側**で、
## 「初期ビルドが中央」の形にはできない——札で伸びる一方の軸だから。
## 上端は実測(playtest/measure_offense_index.gd, greedy報酬で1200ラン)の到達分布から
## 取ってある: 攻め力 p50 2.25 / p90 2.79 / p99 3.31 / 最大 4.19、
## 弾き p50 1.51 / p90 1.69 / p99 1.87 / 最大 2.17。**実測の最大がちょうど満タン**に
## なる位置に置いた(p90を上端にすると1割のランが振り切れたまま最後まで動かない)。
## 2つで出発点の埋まり具合が違う(攻め力0.29・弾き0.45)のは見た目の不揃いではなく、
## 弾きの方が札で伸びにくいという実測そのもの。
const ATTACK_INDEX_MAX := 3.5
const KNOCKBACK_INDEX_MAX := 2.2
...
static func rows(stats: SpinnerStats, ghost_seconds: float = 0.0) -> Array[Dictionary]:
	var r: Array[Dictionary] = [
		...
		{
			"label_key": "STAT_ATTACK",
			"fraction": _fraction(attack_index(stats), ATTACK_INDEX_MAX),
		},
		{
			"label_key": "STAT_KNOCKBACK",
			"fraction": _fraction(knockback_index(stats), KNOCKBACK_INDEX_MAX),
		},
...
## 攻め力の倍率。**素のコマ(ラン開始時)が同じ相手へ与える削りの何倍か**で、
## 出発点がちょうど 1.00。報酬カードの攻め力の行(PartPreview.attack_index)と同じ定義。
##
## **なぜビルド表示に攻めの行が要るのか**(2026-08-08 夜)
## 報酬カードは 硬さ→攻め力→弾き→打たれ強さ→壁強さ の5行で札を約束する。ところが
## 札を取った後に残る常設のビルド表示(StatPanel)には 硬さ・寿命・重さ・大きさ・反発・
## 初期回転 しか無く、**攻めの2行だけがどこにも出ない**。2026-08-02 のサイクルが
## 「カードが約束した硬さ・寿命が取った後どこにも出ない」を塞いだのと同型の切れ方が、
## そのまま攻めの側に残っていた(硬さ・打たれ強さ・壁強さは 2026-08-02/08-06 に
## この行と報酬カードで揃えてある)。
##
## 一次証拠(コールドプレイ 2026-08-08 夜, seed=4821・9戦全勝でクリア):
## 11枚のうち **SHARP_EDGE 2枚・DRILL_BIT 3枚** と半分近くを攻めの札に使い、
## 報酬画面の攻め力は ×1.00 → ×4.84 まで伸びたのに、**取った瞬間にその数字は消える**。
## 次の報酬画面まで、自分の攻めが伸びたのかどうかを確かめる場所が1つも無かった。
## EDGE/DRILL は重さも大きさも反発も回転も1ミリも動かさないので、
## **生の4本+硬さ+寿命の6行が全部据え置きのまま**札だけが増えていく。
##
## **なぜ基準の相手を素のコマに固定するのか**
## 報酬カードは「次に選べる部屋のいちばん硬い1体」を基準に出す。あれは
## 「次の部屋にこの札が効くか」を答える画面なので正しいが、**常設HUDで同じことを
## すると部屋を移るだけで値が跳ねる**——貫通削り(pierce)は相手の硬さに依らないので
## 比が硬い相手ほど開き、今回のコールドプレイでも基準が決戦の敵に変わった瞬間
## ×4.84 → ×11.92 と2.5倍になった。StatPanelは動いた行を光らせるので、それは
## **札を1枚も取っていないのにバーが光る**=嘘になる。相手を素のコマに固定すれば、
## この行は**自分のビルドが動いたときだけ**動く——rows() の他の6行と同じ性質になる。
## 部屋ごとの比べ合いは enemy_rows の取り分バーが別に持っている。
##
## 素のコマは `SpinnerStats.default_player()`(PartPreview.attack_index の分母と
## 同じ実体)。相手も同じ素のコマなので、分母・分子とも硬さ 0.735 で割る。
static func attack_index(stats: SpinnerStats) -> float:
	return PartPreview.attack_index(stats, PartPreview.toughness(SpinnerStats.default_player()))


## 弾きの倍率。**素のコマが同じ相手を弾く量の何倍か**で、出発点がちょうど 1.00。
## 攻め力が削って倒す軸なのに対し、こちらは壁へ押し込んで倒す軸。
## 報酬カードの弾きの行(PartPreview.knockback_index)と同じ定義で、基準の相手を
## 素のコマへ固定するのは attack_index と同じ理由。
##
## この行が要る一次証拠は死に方の側にある。敵の死因は Lv2以降で1/4〜1/2以上が壁、
## Lv3・Lv4 の乱戦では壁が削りを上回る(report.md の死因内訳)。今回のコールドプレイでも
## 段1・段4・段8 の3戦が壁決着で、段8の敵のrps喪失は 壁22.1/19.7 対 削り15.2/16.3 と
## 壁が主因だった。**実際に勝たせている軸**なのに、押し込む力が伸びたのかどうかは
## 報酬カードを閉じた瞬間に見えなくなっていた。
static func knockback_index(stats: SpinnerStats) -> float:
	return PartPreview.knockback_index(stats, SpinnerStats.default_player())
