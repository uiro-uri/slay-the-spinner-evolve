...
func _test_single_escape_guarantee(check: Callable) -> void:
	...
	# 段5以降のdemoteだけは2045個、段5以降の交換は2414個。斥候(単体エリート)
	# 導入後は、乱戦ロール自体が斥候に置き換わるぶんが減って2201個
	# (斥候率0.2×乱戦率0.4×約13ノード×200生成 ≈ 200個減の理論値どおり)。
	# 斥候段に段1を足して(2026-08-07)2204個。ほぼ動かないのは、増えた段が保証の
	# 対象外(段5未満)で交換の側に触れないうえ、段1は元々3ノードしかないから。
	# 2100は交換実装(2204)とdemoteだけ(2045以下)の分水嶺。
	check.call(
		multi_total >= 2100,
