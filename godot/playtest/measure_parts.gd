...
##
##   godot --headless --path godot --script res://playtest/measure_parts.gd -- [--count=800]
##
...
func _init() -> void:
	...
	print("# 単独パーツの1戦あたり勝率 (発射=intercept, count=%d/セル)" % count)
	...
	for level in LEVELS:
		...
		var base_rate := _win_rate(_stats_with(0, 0), 0.0, pool, count)
		...
		for pid in PART_IDS:
			...
			for copies in COPIES:
				...
				var rate := _win_rate(_stats_with(pid, copies), ghost, pool, count)
				rates.append(rate)
...
func _win_rate(stats: SpinnerStats, ghost: float, pool: Array[EnemyData], count: int) -> float:
	var wins := 0
	for i in count:
		...
		var record := BattleSim.play_one(
			i, enemies, LaunchPolicy.Kind.INTERCEPT, stats, null, null, ghost
		)
		if record["win"]:
			wins += 1
