...
static func play_one(
	...
	while true:
		...
		while true:
			...
			# 実プレイのリトライは「同じ相手に挑む/相手を替えて挑む」の2択だが
			# (Main._on_continue_requested、規則はRetryPlan)、ボットはここで
			# 常に**替える方**を踏む。ボットには前の敗北の記憶が無く、据え置いても
			# 自分の発射抽選が引き直されるだけで「反省」に相当するものが無いためで、
			# 据え置きの価値は人間にしか出ない。同レベルの別個体になる点は変わらないので
			# 段ごとの難度は保たれる。
			if continues > 0:
				continues -= 1
				continues_used += 1
...
## hit_guardは衝突削りを(1-hit_guard)倍にする。greedyがGUARD札を値踏みできるよう
## 式に織り込む(織り込まないとGHOST同様「絶対選ばれない札」になり統計に現れない)。
## 倍率は被弾数換算の1/(1-g)ではなく控えめな線形(1+g)にする: 1/(1-g)は逓増で、
## greedyが上限まで GUARD を連続で掴んで RPS/質量を逃し、hit_guardが効かない
