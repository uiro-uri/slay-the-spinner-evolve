...
## このランで自分の回転がどこへ消えたかの累計を、発射前に据え置きで出す
## (RpsLossText.carryover_line)。上の見積もりが**届くまで**しか見ないのに対し、
## こちらは噛み合った後も含む実測。ランの1戦目は積んだものが無いので出ない。
...
func _ready() -> void:
	...
	# 土俵の名前と性格を、狙いを決めている間だけメッセージの直下に出す。
	# 土俵は絵では描き分けてある(壁の形・等高線・柱)のに名前がどこにも出ず、
	# 何度戦っても符丁を覚えられなかった(FieldFlavor 冒頭のコールドプレイ)。
	# 読ませたいのは「どこで戦うか」を決めた**後**ではなく発射を決める前なので、
	# 決着まで空いている内訳行を借りる。_begin() が発射の瞬間に消し、
	# 決着後は内訳が同じ行へ入る——2つの文が同時に出ることはない。
	_loss_label.text = FieldFlavor.line(_field)

	# このランで払ってきた喪失の内訳も、決着まで空いている残り回転の行を借りて
	# 据え置きで出す(土俵の名前・先読みの行と同じ寿命で、_begin() が発射の瞬間に消す)。
	# 発射前の見積もり(WallCostPreview)は相手に届くまでしか見ないので、
	# 噛み合った後に払った壁は実測のこちらでしか出せない(RpsLossText.carryover_line)。
	_remain_label.text = (
		RpsLossText.carryover_line(GameState.run_rps_loss) if show_last_loss else ""
	)
