...
```bash
export GODOT_BIN=$(bash scripts/evolve/bootstrap_godot.sh)
...
ブートストラップスクリプトは公式 GitHub releases からの DL を試し、プロキシに 403 で
弾かれる環境では mirror.gcr.io の godot-ci イメージから公式 4.7.1 バイナリを抽出する
（2026-07-19 のサイクルで実証済みの経路）。スクリプトが壊れていた場合は、この2経路を
手で再現してよい（スクリプトの修正 PR も可。CODEOWNERS 対象外）。
